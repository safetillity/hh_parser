...
function App() {
  ...
  const [numVacancies, setNumVacancies] = useState<number>(20);
  const [region, setRegion] = useState<string>('113');
  ...
  const [regionInput, setRegionInput] = useState<string>('Вся Россия');
  const [showRegions, setShowRegions] = useState<boolean>(false);
  const [filteredRegions, setFilteredRegions] = useState<typeof regions>(regions || []);
  ...
  const handleNumVacanciesChange = (event: Event, value: number | number[]) => {
    setNumVacancies(Array.isArray(value) ? value[0] : value);
  };
  ...
  const handleSort = () => {
    ...
    const sortedVacancies = [...results.vacancies].sort((a, b) => {
      const getMaxSalary = (vacancy: any) => {
  ...
  const handleRegionSelect = (selectedRegion: { id: string; name: string }) => {
  ...
  const handleClickOutside = (event: MouseEvent) => {
    if (!(event.target as HTMLElement).closest('.region-selector')) {
      setShowRegions(false);
    }
  };
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 4, mb: 3 }}>
                <Box sx={{ flex: 1, minWidth: 300 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {Object.entries(originalResults.statistics.salary_stats as Record<string, number>).map(([key, value]: [string, number]) => {
                      let vacancyId = null;
                      if (key === 'min' || key === 'max') {
                        const vacancies = originalResults.vacancies;
                        if (key === 'min') {
                          const minVacancy = vacancies.reduce((min: any, curr: any) => {
                          ...
                          const maxVacancy = vacancies.reduce((max: any, curr: any) => {
                ...
                <Box sx={{ flex: 1, minWidth: 300 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {Object.entries(originalResults.statistics.experience_distribution as Record<string, number>)
                      .sort((a, b) => (b[1] as number) - (a[1] as number))
                      .map(([exp, count]: [string, number]) => (
                ...
                <Box sx={{ flex: 1, minWidth: 300 }}>
                  ...
                  {(() => {
                    ...
                    const topSkillsArray = (Object.entries(skillCounts) as [string, number][]).sort((a, b) => b[1] - a[1]).slice(0, 10);
                    ...
                    return (
                      <>
                        ...
                        {topSkillsArray.map(([skill, count]: [string, number]) => {
            ...
            >
              ...
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {results.vacancies.map((vacancy: any, index: number) => {
                          ...
                          <TableCell>
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                              {Array.isArray(vacancy.key_skills) && vacancy.key_skills.length > 0 ? (
                                vacancy.key_skills.map((skill: string, i: number) => (
                                  <Box
                                    key={i}
                                    sx={{
                                      bgcolor: alpha(theme.palette.primary.main, 0.1),
